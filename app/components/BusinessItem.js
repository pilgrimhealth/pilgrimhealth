...
function BusinessItem({business,showDir=false}) {
    ...
    <div className='min-w-[195px]  flex flex-col justify-between min-h-[110px]   flex-shrink-0 pb-2  px-2
     ...
     hover:scale-110 transition-all   mt-[20px] cursor-pointer z-20'>
         ...
         <Link href="" onClick={(e) => { e.preventDefault(); }} className='text-[14px] font-bold flex  justify-between items-center gap-3` relative  '>{business.name[currentLanguage]} </Link>
          ...
          {showDir?  <div className='border-t-[1px] p-1 mt-1'>
              ...
              border-blue-500
              hover:text-white
              hover:bg-blue-500' onClick={(e)=>{ e.stopPropagation(); onDirectionClick();}} >Get Direction</span></h2>
            </div>:null}
    </div>
  )
}
